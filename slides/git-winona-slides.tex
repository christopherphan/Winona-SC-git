...
\begin{itemize} \item Take turns. \pause \textcolor{red!50!black}{\textbf{Problem}: Each one will spend a lot of time waiting for the other to finish.}
...
\pause
  \item As we have this record of who made what changes when, we know who to ask if we have questions later on, and, if needed, revert to a previous version, much like the ``undo'' feature in an editor.

\pause
\item  When several people collaborate in the same project, it's possible to accidentally overlook or overwrite someone's changes. The version control system automatically notifies users whenever there's a conflict between one person's work and another's.
\end{itemize}
...
\begin{frame}
\frametitle{Why version control?\footnote{\url{http://swcarpentry.github.io/git-novice/01-basics/}}}

\begin{itemize}
\item \url{http://www.phdcomics.com/comics/archive.php?comicid=1531}

\item \url{http://bit.ly/motivate_git}
\end{itemize}
...
\begin{frame}
\frametitle{Format for \mintinline{shell}{git} shell commands\footnote{\url{http://swcarpentry.github.io/git-novice/02-setup/}}}

\texttt{git} \textit{verb}

\vskip 2 em

E.g.:
\begin{itemize}
  \item \mintinline{shell}{git add}
  \item \mintinline{shell}{git commit}
  \item \mintinline{shell}{git push}
  \item \mintinline{shell}{git config}
\end{itemize}
\end{frame}
