...
\usepackage{tikz} % Package for drawing things. I used Inkscape
                  % to convert SVG files to TikZ commands.
\usepackage{minted} % Uses Pygments to highlight
\usepackage{bbding} % Provides the red X and green checkmark
\usepackage{menukeys} % Makes the keycap [Q] look nice
...
Note: If the log it too long, Git will show in a ``pager'' interface. Press \keys{Q} to get out.
...
The string \mintinline{text}{4eece5059b74acd7223fad89d192b262ec41651f} is a unique identifier for the commit (will be different for you).
\pause
\vskip 1 em
...
\begin{enumerate}
\item \begin{minted}[fontsize=\footnotesize]{shell}
$ git checkout HEAD
\end{minted}
\onslide<3->{\wrongans}
\item \begin{minted}[fontsize=\footnotesize]{shell}
...
\begin{itemize}
\item \texttt{git config} --- access configuration
\item \texttt{git init} --- create a (local) repo
\item \texttt{git add} --- add a file to repo or stage changes
\item \texttt{git status} --- check the status of the repo
\item \texttt{git commit} --- commit (staged) changes to repo
\item \texttt{git push} --- push local commits to the remote copy of the repo
\item \texttt{git pull} --- pull commits from remote copy of the repo to the local copy of repo
\end{itemize}
